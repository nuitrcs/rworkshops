...
One point with if...else that trips some people up is that you can't the conditional test in `if()` is a single value -- not a vector.  The following gives a warning and unexpected output:
...
This example created a new variable, but you can save the results back to the same variable if you're trying to change values in place.  
...
for (i in 1:length(y)) {
  if (y[i] < 5) {
    print(paste(y[i], "is less than 5"))
  } else {
    print(paste(y[i], "is >= 5"))
  }
}
```
...
Or from the `psych` package, `describe`:
...
Note that describe tries to treat all of the variables as numeric. (The package is primarly for examining numeric data.)
